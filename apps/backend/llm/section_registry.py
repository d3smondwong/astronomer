...
SECTION_REGISTRY: list[Section] = [
    Section(
        ...
        emphasis=(
            ...
            "classical rules: 论日干格局, 论五行组合, 论正印, the noble-star readings "
            "论太极贵人 / 论天乙贵人 / 论天月德, 释六十甲子, and the 卷六 destiny structures as "
            "top-level 古籍解读 keys (each one appears only when this chart matches it — read those "
            "present, ignore the rest): where the day master's whole chart runs to one element, the "
            "pure-element 专旺 structures (曲直 = benevolent, enduring; 炎上; 从革 = lofty, pure; 润下; "
            "稼穑) plus 一气生成 (four identical stems, 天元一气 — an exceptionally concentrated, "
            "single-minded nature) and the paired-element harmony patterns (金白水清 / 木火交辉 = "
            "refined and brilliant, versus 火土夹杂 / 土局润下 = dull, toiling) read as the deepest "
            "layer of temperament — an unusually singular, focused nature; the character 格 日德 "
            "(benevolent, helped in adversity), 日贵 (pure, virtuous), 福德秀气 (bright, refined), "
            "魁罡 (decisive, commanding, quick-tempered), 八专禄旺 (clever, hardy, sensual), "
            "还魂借气 (绝处逢生 — resilience, an instinct for turning adversity around); 论阳刃 for a "
            "sharp, cutting edge; and 论倒食 (枭神夺食 — a self-undermining streak, 福薄 and unfinished "
            "pursuits) as a blind spot to watch. "
            "The FULL spiritual-star block (神煞) across every pillar — read each star's 解读, both "
            "the auspicious and the malefic; skip neither. Map the gifts (the intellect and "
            "scholarship stars 文昌 / 学堂 / 词馆, the noble and benevolent 太极贵人 / 天乙贵人 / 天月德, "
            "the multi-talented 六秀 / 三奇, the healing 天医, the restless 驿马, the solitary-artistic "
            "华盖) to mind, strengths and drives; map the shadows (亡神, 月厌, the sharp 绞煞 / 剑锋煞 "
            "/ 破煞, the melancholic 元辰 / 吊客) to weakness — reading each from its own 解读, not the "
            "name. "
    ...
    Section(
        ...
        emphasis=(
            ...
            "(parents and siblings): their stems and branches, ten-gods, "
            ...
            "The 卷六 structures as top-level 古籍解读 keys bearing on family (each one appears only when "
            "this chart matches it — read those present, ignore the rest): 论印绶 "
            "(the seal star = mother, and 受父母荫庇 — protection and inheritance from elders) for the "
            "parental bond; ancestral fortune — 岁带正马 (祖业丰厚, a substantial inherited estate) and "
            "日坐天财 (家产殷实, accumulated family wealth) versus 弃印就财 (弃祖基而自创, one who leaves "
            "the ancestral base to build their own); harm to kin — 四位纯全 (六亲刑害, strife among "
            "relatives), 论倒食 (幼克母, early strain on the mother), 墓煞 (亲姻哭送, bereavement in the "
            "family). Take each structure's own 结论 / 现代解读. "
    ...
    Section(
        ...
        emphasis=(
            ...
            "partnership). "
            "The 卷六 structures as top-level 古籍解读 keys bearing on the union (each one appears only "
            "when this chart matches it — read those present, ignore the rest): 论阳刃 (日刃 / 自刃 → "
            "男妨妻女妨夫, a strong-willed partner and marriage friction; when the day master is "
            "instead weak, the same blade can steady the bond) and 论倒食 (长伤妻 — strain on the "
            "spouse) as friction warnings; in a male chart, 时上偏财 and 时带正马 (妻贤子贵 — a capable "
            "wife, with wealth or noble children arriving through her, read from the hour pillar) as "
            "favourable spouse-and-children structures. Where a capable-spouse structure and a "
            "friction one coexist, render the tension, not a one-sided verdict. Take each "
            "structure's own 结论 / 现代解读. "
    ...
    Section(
        ...
        emphasis=(
            ...
            "Classical career rules (古籍解读): the two 卷五 authority chapters, targeted directly "
            "— 论正官 (orthodox authority: management, officialdom and disciplined rank, with 五行官性 "
            "pointing to a fitting field — e.g. a wood authority star suits education and culture) "
            "and 论七杀 (the power star: competitive drive, enforcement and bold command that must be "
            "tamed / 制 to serve, with 五行七杀性 for its field-fit and the 官煞混杂 / 去官留煞 "
            "refinements where both mix); 论日干格局, 论禄神 for income stability, and 论驿马 / 论禄马 "
            "for mobility, travel and "
            ...
            "clashes on the month pillar (career volatility). "
            "The 卷六 destiny structures (as top-level 古籍解读 keys) — the 格局 that most directly judge "
            "rank, calling and talent. Each one appears only when this chart matches it; read "
            "whichever are present, say nothing of the rest. Rank/authority: 财官双美, 魁罡, 论建禄, "
            "论杂气 (透官者贵), 火金铸印 (火炼金成器 — high office, commanding rank), the seal-rescue "
            "patterns 时逢生印 / 胞胎逢印绶 (印 generates or saves the day master into 官贵 — a "
            "strategist's rank), and the self-seated / virtually-drawn authority patterns "
            "(玄武当权 / 朱雀乘风 / 青龙伏形 / 白虎持势 / 勾陈得位 / 飞天禄马 / 倒冲禄马 / 禄元互换 / "
            "虎午奔巳 / 羊击猪蛇 / 六阴朝阳 / 六乙鼠贵 — all 虚取官星, drawing an unseen authority star "
            "into real rank), plus the embraced-nobility 拱禄拱贵 (贵气暗藏, an 官途 opening in "
            "mid-life). "
            "Talent & scholarship: 论伤官 (多艺多能 but 傲物气高), 论食神 / 食神带合 (gifts turned to "
            "real authority), 金白水清 / 木火交辉 / 墨池涌泉 (literary, academic distinction). Working "
            "style: 子遥巳禄 / 丑遥巳禄 (behind-the-scenes, networked), 六壬趋艮 (strategist / advisory), "
            "六甲趋乾 (education, culture, public service), 子午双包 (balance — management, diplomacy, "
            "coordination roles), 冲合禄马 (rising through both competition and alliance — pioneering, "
            "outward-facing work), 日禄归时 (禄 gathering at the hour — standing and success "
            "consolidating in the later career). Career ceiling: 破官 (achievement only through "
            "hidden channels, with unstable rank) and 背禄逐马 (the authority star "
            "fallen into 死绝 — rank that never lands). Take each structure's own 结论 / 现代解读, not "
            "the label."
        ),
    ),
    Section(
        ...
        emphasis=(
            ...
            "release it. Classical rules: 论禄神 (the salary/income star) and the "
            "output-feeding-wealth principle (食伤生财). The output stars (食伤) as the source "
            "feeding wealth; the rob-wealth star (劫财 = competition for or loss of wealth) and "
            "clashes on the wealth star. "
            ...
            "prosperity and livelihood (e.g. 活计生涯, 昌荣 versus 只许平). "
            "The 卷六 wealth structures as top-level 古籍解读 keys (each one appears only when this "
            "chart matches it — read those present, ignore the rest). Sources & capacity: 论正财 "
            "(steady, 富足安稳), 论偏财 / 偏正财合论 (windfall, business, investment), 财旺生官 "
            "(因富致贵 — wealth converting to status), 时上偏财 (later-life outside wealth), 日禄归时 "
            "(禄 gathering at the hour — income and assets consolidating in later life), 壬骑龙背 "
            "(unexpected, self-started fortune), 弃命从财 / 弃印就财 (a chart that surrenders to "
            "wealth, or leaves its inheritance to build its own), 论杂气 (透财者富 — a vault that "
            "releases wealth), 财官双美 (an innate wealth-and-status base), and the innate-fortune "
            "structures 福星贵人 (effortless prosperity without heavy toil) and 红鸾天印 (marked "
            "wealth paired with standing). Windfall versus keeping: "
            "飞财 (wealth bursting from a hidden place) against 破财 (横发横财 — comes and goes, hard "
            "to hold). Leaks & ceilings: 背禄逐马 (the wealth star fallen into 死绝 — 财马逐而不得). "
            "Take each structure's own 结论 / 现代解读, not the label."
        ),
    ),
]
